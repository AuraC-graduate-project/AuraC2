...
CREATE INDEX idx_contests_status_start_time ON contests (status, start_time);
...
CREATE INDEX idx_test_cases_problem_public ON test_cases (problem_id, is_public);
...
CREATE INDEX idx_clarifications_contest_created
    ON clarifications (contest_id, created_at);
CREATE INDEX idx_clarifications_contest_reply_status_created
    ON clarifications (contest_id, reply_type, status, created_at);
CREATE INDEX idx_clarifications_user_contest_created
    ON clarifications (user_id, contest_id, created_at);
CREATE INDEX idx_clarifications_status_created
...
CREATE INDEX idx_submission_judge_results_submission_run
    ON submission_judge_results (submission_id, judge_run_id);

CREATE TABLE scoreboard_reveal_states (
    id BIGSERIAL PRIMARY KEY,
    contest_id BIGINT NOT NULL,
    status VARCHAR(255) NOT NULL,
...
CREATE INDEX idx_scoreboard_reveal_states_status
    ON scoreboard_reveal_states (status);

CREATE TABLE scoreboard_reveal_cells (
    id BIGSERIAL PRIMARY KEY,
...
CREATE INDEX idx_scoreboard_reveal_cells_state_order
    ON scoreboard_reveal_cells (reveal_state_id, reveal_order, id);
CREATE INDEX idx_scoreboard_reveal_cells_state_revealed_order
