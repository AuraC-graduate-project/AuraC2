...
export function useContestStream(
  ...
  useEffect(() => {// Main effect: open SSE once
    ...
    // might not need to... we only have one contest ⛔️
    //Why a snapshot? The snapshot event provides the full current state of all contests immediately upon connection. This ensures that the client has an up-to-date view right away, without waiting for the next incremental update. After receiving the snapshot, the client can then apply subsequent contest-update events to stay in sync with any changes.
    ...
    // what is this function? The handleUpdate function is a callback that gets called whenever the server sends a "contest-update" event through the SSE stream. It receives the event data, attempts to parse it as JSON, and then calls the onContestUpdate handler (if provided) with the parsed data. This allows the client to react to incremental updates about contest changes in real time.
    ...
    //What are these? We register event listeners for both "snapshot" and "contest-update" events on the EventSource. This allows us to handle the initial state of contests as well as any subsequent updates sent by the server. The handlers are typecast to EventListener to satisfy TypeScript's type checking, since our callbacks have specific signatures that differ from the default EventListener type.
    /**
    ...
    //When connection succeeds, UI becomes live. If connection fails, we check if the browser has given up (readyState === CLOSED) or is still trying (readyState === CONNECTING) and update the connection state accordingly. The browser will automatically attempt to reconnect with an exponential backoff strategy, so we don't need to implement that logic ourselves.
    ...
    //This runs when the component unmounts. We clean up by removing the event listeners and closing the EventSource connection to prevent memory leaks and unnecessary network activity.
    return () => {
