...
| Reference-solution oracle and generated tests | Implemented backend/admin API | `ReferenceSolution`, `InputGenerator`, `InputValidator`, `GeneratedTestBatch`, `GeneratedTestCase`, `Counterexample`, `OracleService`, `OracleAdminController`, `V4__reference_oracle_generated_tests.sql`, `OracleServiceTest` | Admins can configure reference/generator/validator programs, execute them through Judge0 with `wait=true`, store generated hidden tests/reference outputs, evaluate a selected submission deterministically, store counterexamples, and promote counterexamples into official hidden `TestCase` rows. | Include as a deterministic hybrid judging extension. State clearly that generated tests do not prove correctness and affect official judging only after promotion and rejudge. |
...
| `Problem` | `id`, `title`, `description`, `timeLimit`, `memoryLimit`, `difficulty`, `comparePolicy`, `floatAbsoluteEpsilon`, `floatRelativeEpsilon`, `validationMode`, `validatorLanguageId`, `validatorSourceHash`, `validatorEnabled` | Many problems belong to one contest; one problem has many test cases and submissions. | Time/memory limits are passed to Judge0. Compare policy controls exact Judge0 expected-output judging or deterministic backend stdout comparison. Custom validator source is stored server-side and hidden from TEAM responses. |
...
13. Custom output validators and the backend/admin reference-solution oracle are implemented through Judge0-sandboxed execution. Generated tests can discover counterexamples and promote them to hidden tests, but they do not prove correctness; interactive judging and ML verdicts remain unsupported.
