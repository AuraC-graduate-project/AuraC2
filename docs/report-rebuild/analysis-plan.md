...
| Structured problem statements and prompt exports | Implemented backend/admin/team UI | `Problem.statement/inputFormat/outputFormat/constraintsText/publicNotes/adminNotes`, `PromptExportService`, `PromptTemplateRenderer`, `PromptVisibilityPolicy`, `SupportedLanguageCatalog`, `ProblemStatementPreview`, `OraclePanel`, `V6__structured_problem_statements.sql` | Problems now support structured contestant-facing statement sections, public notes, and admin-only notes with legacy `description` fallback. Prompt exports are deterministic text only, use Recommended Admin Prompt, Public/Safe Prompt, and Custom Advanced Prompt modes, validate target language server-side, include C++17 compile-safety guardrails for generator/validator prompts, and never call external AI APIs. | Include as problem-authoring and problem-engineering support. Emphasize no AI runtime dependency and server-side visibility enforcement. |
| Contestant-safe statement PDF exports | Implemented backend/admin UI | `ProblemStatementPdfService`, `ContestantSafeStatementModel`, `StatementPdfRenderer`, `ProblemStatementPdfController`, `ProblemsView`, `ProblemStatementPdfServiceTest` | Admins can export a single problem PDF or contest booklet PDF. Exports build a contestant-safe statement model first, use a Codeforces-style section order with examples before notes, render public samples as grouped monospace input/output blocks, and include structured statement fields plus public samples only; hidden tests, admin notes, source code, and diagnostics are excluded. Booklet export directly concatenates the same single-problem statement layout, starts each problem on a new page, and has no cover page by default. The PDF renderer is text-focused and flattens rich text into printable contest-statement sections. | Include as implemented admin export, with limitation that rich HTML is converted to plain printable text. |
| Contestant Run workflow | Implemented backend/team UI | `TeamRunController`, `TeamRunService`, `UserCustomTestCase`, `UserCustomTestCaseRepository`, `OracleJudge0ExecutionService`, `TeamWorkspace`, `ProblemStatementPanel`, `CodeEditor`, `teamApi`, `V9__team_custom_run_tests.sql`, `TeamRunServiceTest` | TEAM users can run current code beside Submit against public samples and all owner-scoped custom tests for the selected problem by default. The Test Cases tab is the single team-facing place for locked public samples, private custom-test add/edit/delete, and transient per-case run results. Runs use Judge0 synchronously, compare public/user-provided expected output with the problem compare policy or custom output validator, validate custom input with the active input validator when present, and show compilation errors once at run level with selected cases marked not run. Run does not create `Submission` rows, publish queue messages, change scoreboard/penalty state, expose hidden tests, expose hidden expected output, or run the reference solution as an oracle for custom input. | Add as a non-scoring team workflow separate from official judging. |
| Contest-scoped team moderation | Implemented backend/admin/team UI | `ContestTeamModeration`, `ContestModerationAuditLog`, `ContestTeamModerationService`, `AdminTeamModerationController`, `TeamContestAccessController`, `ContestModerationCsvExporter`, `TeamsView`, `TeamContestGate`, `SubmissionService`, `TeamRunService`, `ScoreboardService`, `V10__contest_team_moderation.sql`, moderation service/export/security tests | The Teams page now separates global account management from contest-specific moderation and logs. Admins can select a contest, hide/show teams on the scoreboard, disqualify/restore teams, disable/enable Submit, disable/enable Run, require reasons for sensitive actions, inspect audit logs, and export filtered spreadsheet-safe CSV. Submit and Run are guarded server-side; disqualified teams see a blocked contest page; scoreboard snapshots exclude hidden/disqualified teams while preserving stored submissions. | Include as contest moderation, emphasizing contest scope, auditability, and that global team accounts and stored submissions are not deleted. |
...
| Frontend team | `team/App.tsx`, `TeamWorkspace`, `ProblemStatementPanel`, `CodeEditor`, `ProblemSidebar`, `SubmissionHistory`, `Header`, `Clarifications`, `useCodeDraft` | Team active-contest workspace, compact contest header/controls, code draft persistence, non-scoring Run results on test-case cards, official submissions drawer, problem sidebar, timer, and clarification workflow. |
...
| `/api/team/problems/{problemId}/custom-tests` | GET/POST | TEAM | Implemented owner-scoped custom tests | `TeamRunController`, `TeamRunService`, `UserCustomTestCase` |
| `/api/team/problems/{problemId}/custom-tests/{customTestId}` | PUT/DELETE | TEAM | Implemented owner-scoped edit/delete only | `TeamRunController`, `TeamRunService` |
| `/api/team/problems/{problemId}/run` | POST | TEAM | Implemented non-scoring run over public samples and owner-scoped custom tests | `TeamRunController`, `TeamRunService`, `CodeEditor`, `ProblemStatementPanel` |
...
| Contestant Run execution | Implemented | `TeamRunService`, `OracleJudge0ExecutionService`, `OutputComparator`, `CustomValidatorService`, `InputValidatorRepository` | Non-scoring runs use synchronous Judge0 calls and return transient per-case results. Compilation errors are returned as one run-level compile error with selected cases marked skipped. The flow does not use RabbitMQ, does not create `Submission` rows, and does not publish scoreboard-affecting events. |
...
| Team workspace | `team/App.tsx`, `TeamWorkspace`, `Header`, `ProblemSidebar`, `ProblemStatementPanel`, `CodeEditor`, `SubmissionHistory`, `useSubmissionStream` | Implemented with ongoing polish | Uses active contest, compact contest controls, problem statement/test-case tabs, code editor, non-scoring Run workflow, official submissions, draft persistence, and live submission updates. |
...
17. Statement PDF exports are intentionally contestant-safe and printable. They use a dedicated contestant-safe model and grouped monospace sample blocks, but rich text is flattened rather than reproduced as full HTML layout.
...
20. Contestant Run is non-scoring and owner-scoped, but it is not a persistent run-history subsystem; execution is synchronous through Judge0, protected by a small in-memory rate limit, and shown as transient state on Test Cases tab cards.
